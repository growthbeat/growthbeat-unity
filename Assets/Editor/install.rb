...
library_path = 'https://github.com/SIROK/growthbeat-ios/archive/latest.zip'
...
if application_id && secret then
	initialize_code = "[[Growthbeat sharedInstance] initializeWithApplicationId:@"P5C3vzoLOEijnlVj" credentialId:@"btFlFAitBJ1CBdL3IR3ROnhLYbeqmLlY"];"
	edit_source = true
end
download_library = (/^https?:\/\// =~ library_path)

project_file_suffix = '.xcodeproj'
framework_name = 'Growthbeat.framework'
framework_dependencies = []
import_code = '#import <Growthbeat/Growthbeat.h>'
...
# Add Growthbeat.framework and some frameworks.
if !include_framework?(target, framework_name) then
	...
	error("Growthbeat.framework already added.")
end
...
# Add EasyGrowthbeat call to AppDelegate
...
# Add Growthbeat import statement to prefix header
...
puts "* Install and link Growthbeat.framework"
puts "* Add $(inherit) and $(SRCROOT) to framework search paths"
if edit_source then
	puts "* Import Growthbeat.h in prefix header"
end
...
if download_library then
	puts "Downloading Growthbeat.framework..."
	system "mkdir -p /tmp/Growthbeat && curl -fsSL #{library_path} -o /tmp/Growthbeat/growthbeat-ios.zip && unzip -qo /tmp/Growthbeat/growthbeat-ios.zip -d /tmp/Growthbeat/"
	library_path = '/tmp/Growthbeat/growthbeat-ios/Growthbeat.framework'
end

puts "Copy Growthbeat.framework to project..."
